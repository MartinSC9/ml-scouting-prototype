    ...
    <AdminLayout activeItem="referidos" title="Referidos" subtitle="Programa de referidos" level={3}>
      <section className="p-8 space-y-8">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100">
          <h3 className="font-bold text-[#0A1A3A] mb-4">Top Referidores</h3>
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              <div key={r.rank} className="bg-[#f7f9fc] p-4 rounded-lg flex items-center gap-3">
                <div className={`w-10 h-10 ${r.bg} rounded-full flex items-center justify-center`}><span className={`${r.color} font-bold text-sm`}>{r.rank}</span></div>
                <div><p className="text-sm font-medium text-[#0A1A3A]">{r.name}</p><p className="text-xs text-slate-500">{r.info}</p></div>
              </div>
            ))}
          </div>
        </div>
